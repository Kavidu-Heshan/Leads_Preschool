...
const EventPhotoUpload = () => {
  ...
  const fetchAllPhotos = async () => {
    try {
      const response = await axios.get("http://localhost:3002/event-photos");
  ...
  const fetchEventPhotos = async (eventId) => {
    try {
      const response = await axios.get(`http://localhost:3002/event-photos/event/${eventId}`);
      setEventPhotos(response.data);
    } catch (err) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3002/event-photos/upload", formDataToSend, {
        headers: {
  ...
  const handleDeletePhoto = async (photoId) => {
    if (window.confirm("Are you sure you want to delete this photo?")) {
      try {
        const response = await axios.delete(`http://localhost:3002/event-photos/${photoId}`);
        if (response.data.success) {
  ...
  const handleSetFeatured = async (photoId) => {
    try {
      const response = await axios.patch(`http://localhost:3002/event-photos/${photoId}/featured`);
      if (response.data.success) {
