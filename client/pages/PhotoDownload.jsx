...
const PhotoDownload = () => {
  ...
  const fetchPastEvents = async () => {
    try {
      setLoading(true);
      const response = await axios.get("http://localhost:3002/events/past/list");
  ...
  const fetchEventPhotos = async (event) => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:3002/event-photos/event/${event.eventId}`);
